 ...
 * @return			array
 */
function g1_comment_form_default_fields( $fields ) {
	$commenter = wp_get_current_commenter();
	$req = get_option( 'require_name_email' );
			
	$fields['author'] = 
		'<div class="form-row comment-form-author">' .
			'<label for="author">'.__( 'Name', 'g1_theme' ).
			( $req ? ' <em class="g1-meta">'.__('(required)', 'g1_theme').'</em>' : '' ).
			'</label>'.
	...
	$fields['email'] =
		'<div class="form-row comment-form-email">'.
        	'<label for="email">'.__( 'Email', 'g1_theme' ).
            ( $req ? ' <em class="g1-meta">'.__('(required)', 'g1_theme').'</em>' : '' ).'</label>'.
    ...
    $fields['url'] =
    	'<div class="form-row comment-form-url">'.	 
        	'<label for="url">'.__( 'Website', 'g1_theme' ).'</label>' .
 ...
 * @return			string
 */
function g1_comment_form_field_comment( $field ) {
	$field =
    	'<div class="form-row comment-form-comment">' .
	        '<label for="comment">' . __( 'Comment', 'g1_theme' ) . ' <em class="g1-meta">'.__('(required)', 'g1_theme').'</em></label>' .
 ...
 * @return			array
 */
function g1_comment_form_defaults( $defaults ) {
