...
?> <h2 id="woocommerce-firstdata-my-payment-methods"><?php _e( 'My Saved Cards', WC_FirstData::TEXT_DOMAIN ); ?></h2><?php
			...
			<?php foreach ( $tokens as $token => $payment ) :
				...
				<tr class="wc-firstdata-payment-method-label">
					...
					<td class="wc-firstdata-payment-method-actions" style="width: 1%; text-align: center;">
						<a href="<?php echo esc_url( $delete_url ); ?>" class="wc-firstdata-delete-payment-method"><img src="<?php echo esc_url( SV_WC_Plugin_Compatibility::force_https_url( $wc_firstdata->get_plugin_url() ) . '/' . $wc_firstdata->get_framework_image_path() . 'cross.png' ); ?>" alt="[X]" style="box-shadow: none;" /></a>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
