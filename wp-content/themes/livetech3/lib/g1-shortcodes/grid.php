...
function g1_shortcode_grid_wrapper_cb( $content ) {
    $content = preg_replace(
        '/<\/li><\!-- -->\s+<li class="g1-column/i',
        '</li><!-- --><li class="g1-column',
        $content,
        -1
    );
...
add_filter( 'the_content', 'g1_shortcode_grid_wrapper_cb', 11 );
add_filter( 'widget_text', 'g1_shortcode_grid_wrapper_cb', 99 );
...
/**
 * [fix_grid] shortcode callback function.
 *
 * @param array $atts
 * @param string $content
 * @return string
 */
function g1_shortcode_fix_grid( $atts, $content = null ) {
    $content = preg_replace('#^<\/p>|<p>$#', '', $content);

    $out = g1_shortcode_grid_wrapper_cb( do_shortcode( shortcode_unautop( $content ) ) );

    return $out;
}
add_shortcode( 'fix_grid', 'g1_shortcode_fix_grid' );
...
class G1_Grid_Shortcode extends G1_Shortcode {
    ...
    protected function do_shortcode() {
        ...
        $out = 	$ul_ .
                    '<li class="' . sanitize_html_classes( $final_class ) . '"'.$data_delay.'>' .
