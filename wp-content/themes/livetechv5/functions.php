...
{
	if(is_admin()) {
		$price = ( ( is_numeric( $to ) ) ? wc_price( $to ) : $to ) . ' (MSRP: <del>' . ( ( is_numeric( $from ) ) ? wc_price( $from ) : $from ) . '</del>)';
		return $price;
	}
...
function filter_woocommerce_price_html($price, $this)
{
	...
	return '<span class="sale-price"><span class="tablet-hide">Your </span>Price: <ins>' . $price . '</ins></span>';
};
...
add_filter('woocommerce_downloadable_file_allowed_mime_types', 'filter_mime_types');
function filter_mime_types ($mimetypes) {
...
function action_wp_enqueue_scripts() {
	wp_enqueue_style( 'FlatsomeCustomCSS', 'https://www.mylive-tech.com/wp-content/themes/livetechv5/theme-css-processer.php');
}

add_filter('woocommerce_stock_html', 'filter_woocommerce_stock_html', 10);
function filter_woocommerce_stock_html($html, $status, $product) {
	if(strstr($html,"in-stock")) {
		$html = substr($html,0,-4) . '<br/><img src="https://www.mylive-tech.com/wp-content/uploads/2016/03/icon-instock02.png" width="100" height="30" /></p>' ;
	};
	return $html;
}
...
function action_init() {
	remove_action_setup( 'wp_head', 'flatsome_custom_css');
