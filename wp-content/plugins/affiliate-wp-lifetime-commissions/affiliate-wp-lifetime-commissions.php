...
final class AffiliateWP_Lifetime_Commissions {
	 ...
	 * @return The one true AffiliateWP_Lifetime_Commissions
	 */
	...
	private function hooks() {
		...
		add_filter( 'affwp_was_referred', array( $this, 'force_was_referred' ) );
	}
	...
	/**
	 * Forces the was_referred() function to return true so our referral can still be created
	 * @todo  this may not be the best way but means we don't have to setup multiple integrations
	 * @since  1.0
	 */
	public function force_was_referred( $bool ) {
		$bool = true;

		// The insert_pending_referral() function requires an affiliate ID to create a referral so we can safely set this to true.
		return $bool;
	}
