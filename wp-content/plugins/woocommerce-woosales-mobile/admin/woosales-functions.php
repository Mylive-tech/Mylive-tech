...
*/
	...
	$ordersTotal = $wpdb->get_var("SELECT count(*) FROM {$wpdb->prefix}posts WHERE post_type='shop_order' AND post_status != 'trash'");
	...
	for ($i = 0; $i <= 7; $i++) {
		$sql .= "SELECT SUM(postmeta.meta_value) AS 'OrderTotal', COUNT(*) AS 'OrderCount', '".${"today".$i}."' AS 'Text'
				...
				WHERE meta_key='_order_total' AND post_type = 'shop_order' AND post_status != 'trash' AND DATE(posts.post_date) = '".${"today".$i}."' ";
	...
	for ($i = 0; $i <= 6; $i++) {
		$sql .= "SELECT SUM(postmeta.meta_value) AS 'OrderTotal', COUNT(*) AS 'OrderCount','".${"month".$i."y"}."-".${"month".$i}."' AS 'Text'
				FROM {$wpdb->prefix}postmeta as postmeta 
				LEFT JOIN  {$wpdb->prefix}posts as posts ON posts.ID=postmeta.post_id
				WHERE meta_key='_order_total' AND post_type = 'shop_order' AND post_status != 'trash' AND YEAR(DATE(posts.post_date)) = '".${"month".$i."y"}."' AND MONTH(DATE(posts.post_date)) = '".${"month".$i}."' ";
	...
	for ($i = 0; $i <= 1; $i++) {
		$sql .= "SELECT SUM(postmeta.meta_value) AS 'OrderTotal', COUNT(*) AS 'OrderCount','".${"year".$i}."' AS 'Text'
				FROM {$wpdb->prefix}postmeta as postmeta 
				LEFT JOIN  {$wpdb->prefix}posts as posts ON posts.ID=postmeta.post_id
				WHERE meta_key='_order_total' AND post_type = 'shop_order' AND post_status != 'trash' AND YEAR(DATE(posts.post_date)) = '".${"year".$i}."' ";
	...
	for ($i = 1; $i <= 12; $i++) {
		$sql .= "SELECT SUM(postmeta.meta_value) AS 'OrderTotal', '".$year0."-".${"monthy".$i}."' AS 'Text'
				FROM {$wpdb->prefix}postmeta as postmeta 
				LEFT JOIN  {$wpdb->prefix}posts as posts ON posts.ID=postmeta.post_id
				WHERE meta_key='_order_total' AND post_type = 'shop_order' AND post_status != 'trash' AND YEAR(DATE(posts.post_date)) = '".$year0."' AND MONTH(DATE(posts.post_date)) = '".${"monthy".$i}."' ";
	...
	for ($i = 1; $i <= 12; $i++) {
		$sql .= "SELECT SUM(postmeta.meta_value) AS 'OrderTotal', '".$year1."-".${"monthy".$i}."' AS 'Text'
				FROM {$wpdb->prefix}postmeta as postmeta 
				LEFT JOIN  {$wpdb->prefix}posts as posts ON posts.ID=postmeta.post_id
				WHERE meta_key='_order_total' AND post_type = 'shop_order' AND post_status != 'trash' AND YEAR(DATE(posts.post_date)) = '".$year1."' AND MONTH(DATE(posts.post_date)) = '".${"monthy".$i}."' ";
	...
	for ($i = 0; $i <= 1; $i++) {
		...
		$sql = '';
		$sql = "SELECT * FROM {$wpdb->prefix}posts as posts WHERE posts.post_type = 'shop_order' AND posts.post_status != 'trash' ORDER BY posts.post_date DESC LIMIT 10";
...
//echo $item['variation_id'].' - '.$item['line_subtotal'];
			    //print_r($item);
			    if ($product_variation_id > 0) {
				    //echo $product_variation_id.'<br />';
				    $variation = wc_get_product($product_variation_id);
					//echo $variation->get_formatted_name().'<br />';
				    //print_r($variation->get_variation_attributes());
				    $lala = $variation->get_variation_attributes();
				    $lala1 = array_values($lala);
				    $vari = array_shift($lala1);
				    
				    /*
				    echo $vari.'<br />';
				    echo 'U+d0b2U+d18bU+d0b1U+d0beU+d180'.'<br />';
					$string = str_replace('%', 'U+', $vari);
					echo htmlentities($string, ENT_IGNORE, 'UTF-8').'<br />';
					*/
					
					
				    $fullname = $item['name'].' - '.urldecode($vari);
				    /*
				    $variation = wc_get_product($product_variation_id);
					echo $variation->get_formatted_name().'<br />';
					*/
				...
				'productQty' => intval($item['qty']),
				'productPrice' => floatval($item['line_subtotal']),
				'productTotal' => floatval($item['line_total'])
				);
			}
			/*
