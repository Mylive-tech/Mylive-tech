	...
	static function parseProductColumn( $column, $item, $product, $profile ) {
		...
		switch ( $column ) {
			...
			case 'main_offer_image': // BookLoader template
				...
				// // maybe fall back to parent variation featured image (disabled to avoid the same swatch image for all child variations - ticket #6662)
	...
	static function parseFbaSubmissionColumn( $column, $post_id, $_order, $order_item, $listing ) {
		...
		switch ( $column ) {
			...
			case 'AddressFieldOne':
				$has_company = ! empty( $_order->shipping_company );
				$value = $has_company ? $_order->shipping_company   : $_order->shipping_address_1;
				break;
			
			case 'AddressFieldTwo':
				$has_company = ! empty( $_order->shipping_company );
				$value = $has_company ? $_order->shipping_address_1 : $_order->shipping_address_2;
				break;
			
			case 'AddressFieldThree':
				$has_company = ! empty( $_order->shipping_company );
				$value = $has_company ? $_order->shipping_address_2 : '';
				break;
