	...
	*/
	...
	foreach( $business_contact_details as $order => $details ) {
		...
		if ( 'phone' == $details['key'] && $atts['show_phone'] && ! empty( $business_phone ) ) {
			$details_output .= sprintf( '<span class="wpseo-phone">%s: <a href="tel:' . $business_phone . '" class="tel"><span itemprop="telephone">' . $business_phone . '</span></a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' ), $details['label'] );
		}
		if ( 'phone_2' == $details['key'] && $atts['show_phone_2'] && ! empty( $business_phone_2nd ) ) {
			$details_output .= sprintf( '<span class="wpseo-phone2nd">%s: <a href="tel:' . $business_phone_2nd . '" class="tel">' . $business_phone_2nd . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' ), $details['label'] );
		}
