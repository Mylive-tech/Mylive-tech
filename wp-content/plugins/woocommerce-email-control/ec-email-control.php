 ...
 * Define Constants
 */
define( 'WC_EMAIL_CONTROL_VERSION', '2.35' );
		...
		if ( in_array( 'emails', explode( '/', $template_name ) ) && ! in_array( 'plain', explode( '/', $template_name ) ) ) {
			...
			if ( is_array( $ec_email_templates ) && isset( $ec_template_selected ) && $ec_template_selected !== false ) {
				if ( array_key_exists( $ec_template_selected, $ec_email_templates ) ) {
					...
					// if ( ! version_compare( $woocommerce->version, '2.4', '>=' ) ) $this_template_versions = '-below-wc2.4';
					if ( ! version_compare( $woocommerce->version, '2.5', '>=' ) ) $this_template_versions = '-below-wc2.5';
					...
					if ( ! $this_template ) {
						$this_template = trailingslashit( plugin_dir_path(__FILE__) ) . 'templates' . $this_template_versions . '/' . 'emails' . '/' . $this_modified_file_name;
						if ( !file_exists($this_template) ) $this_template = false;
					}
					// Check template-plugin for `templates/emails/supreme/email-footer.php`.
					if ( ! $this_template ) {
						$this_template = trailingslashit( plugin_dir_path(__FILE__) ) . 'templates' . $this_template_versions . '/' . 'emails' . '/' . $ec_template_selected . '/' . $this_original_file_name;
						if ( !file_exists($this_template) ) $this_template = false;
					}
