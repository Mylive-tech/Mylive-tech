...
class WPLA_ProductBuilder {
		...
		if ( count($data['images']) > 1 ) {
			for ($i=1; $i < count($data['images']) ; $i++) { 
				...
				if ( get_option('wpla_import_safe_import_mode', 0) == 0 ) {
					...
					$attachment_id = $this->addProductImage( $post_id, $data['images'][$i], $data['name'], $data['asin'].'-'.($i+1), false );
					if ( $attachment_id ) $image_attachment_ids[] = $attachment_id;
	...
	function addVariation( $post_id, $variation, $data ) {
		...
		if ( $variation->variation_image ) {
			...
			$attachment_id = $this->addProductImage( $variation_id, $variation->variation_image, $variation_post_title, $data['asin'].'-'.$variation_id, true );
	...
	public function addProductImage( $post_id, $image, $title = 'default', $asin = false, $is_featured_image = true ) {
