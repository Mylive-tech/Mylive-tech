...
class WPLA_CustomUpdater {
	...
	public function check_for_plugin_update( $checked_data ) {
		...
		// $wpla_logger->info('check_for_plugin_update('.$this->plugin_slug.'): '.$this->api_url);
		// $wpla_logger->info('raw_response: '.print_r($raw_response,1));
		// $wpla_logger->info('response: '.print_r($response,1));
		// $wpla_logger->info('checked_data: '.print_r($checked_data,1));
	...
	public function plugin_api_call( $def, $action, $args ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false )
				$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred','wpla' ), $request['body'] );
		}
	...
	public function activate_license( $license_key, $email ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false )
				$res = new WP_Error( 'activate_license_failed', __( 'An unknown error occurred','wpla' ), $request['body'] );
		}
	...
	public function deactivate_license( $license_key, $email ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false ) {
				$res = new WP_Error( 'deactivate_license_failed', __( 'An unknown error occurred','wpla' ), $request['body'] );
			}
			
			
			// fallback to return raw request body
			if ( ! $res ) $res = $request['body'];
	...
	public function check_license( $license_key, $email ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false )
				$res = new WP_Error( 'check_license_failed', __( 'An unknown error occurred','wpla' ), $request['body'] );
		}
