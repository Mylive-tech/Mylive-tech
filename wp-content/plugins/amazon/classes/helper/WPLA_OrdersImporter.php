...
class WPLA_OrdersImporter {
	...
	function createOrUpdateWooCommerceOrder( $id ) {
		...
		if ( in_array( $wpla_order['status'], array( 'Canceled', 'Pending' ) ) ) {
			$this->logger->info( 'skipped woo order creation - status is '.$wpla_order['status'].' - order id #'.$wpla_order['order_id'] );				
			return;
		} 
		...
		if ( ! is_array( $wpla_order['items'] ) ) {
			$this->logger->info( 'skipped woo order creation - order line items are invalid (request throttled?)' );
			return;
		}
		...
		$history_details = array( 'post_id' => $order_post_id, 'status' => $wpla_order['status'] );
		$this->addHistory( $wpla_order['order_id'], 'create_order', $history_message, $history_details );
	...
	function processListingItem( $item, $order ) {
		...
		if ( ! $listing ) {
			$history_message = "Skipped foreign item #{$asin}";
			$history_details = array( 'asin' => $asin );
		...
		if ( $order->FulfillmentChannel == 'AFN' ) {
			...
			// add history record
			$history_message = "FBA quantity reduced by $quantity_purchased for listing #{$asin} - FBA stock $fba_quantity ($quantity_sold sold)";
			$history_details = array( 'fba_quantity' => $fba_quantity, 'asin' => $asin, 'qty_purchased' => $quantity_purchased, 'listing_id' => $listing->id );
			...
			// add history record
			$history_message = "Quantity reduced by $quantity_purchased for listing #{$asin} - new stock: $quantity_total ($quantity_sold sold)";
			$history_details = array( 'newstock' => $quantity_total, 'asin' => $asin, 'qty_purchased' => $quantity_purchased, 'listing_id' => $listing->id );
			$this->addHistory( $order_id, 'reduce_stock', $history_message, $history_details );
		...
		// 		array( 'asin' => $asin ) 
		// 	);
		// 	$this->logger->info( 'marked item #'.$asin.' as SOLD ');
		// }
		...
		if ( get_option( 'wpla_sync_inventory' ) == '1' ) {
			...
			if ( ! $listing->post_id ) {
				...
				// add history record
				$history_message = "No product found for $asin (".$listing->status.")";
				$history_details = array( 'asin' => $asin, 'id' => $listing->id );
				$this->addHistory( $order_id, 'skipped_product', $history_message, $history_details );
			...
			$this->logger->info( 'reduced product stock for #'.$post_id.' by '.$quantity_purchased.' - new qty: '.$newstock );
			...
			$history_details = array( 'product_id' => $post_id, 'variation_id' => null, 'newstock' => $newstock, 'qty_purchased' => $quantity_purchased );
			$this->addHistory( $order_id, 'reduce_stock', $history_message, $history_details );
