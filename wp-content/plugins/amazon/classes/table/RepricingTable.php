...
class WPLA_RepricingTable extends WPLA_ListingsTable {
    ...
    function get_bulk_actions() {
        $actions = array(
            ...
            'get_compet_price'    => __('Fetch latest Buy Box pricing info','wpla'),
    ...
    function get_views(){
        $views    = array();
        $current  = ( !empty($_REQUEST['repricing_status']) ? $_REQUEST['repricing_status'] : 'all');
        ...
        $class = ($current == 'is_lowest_price' ? ' class="current"' :'');
        ...
        $class = ($current == 'is_not_lowest_price' ? ' class="current"' :'');
        ...
        $class = ($current == 'no_price_range' ? ' class="current"' :'');
        ...
        $class = ($current == 'pnq_in_process' ? ' class="current"' :'');
        ...
        $class = ($current == 'pnq_failed' ? ' class="current"' :'');
        ...
        // In Stock
        $is_in_stock_url = add_query_arg( 'repricing_status', 'is_in_stock', $base_url );
        $class = ($current == 'is_in_stock' ? ' class="current"' :'');
        $views['is_in_stock'] = "<a href='{$is_in_stock_url}' {$class} >".__('In Stock','wpla')."</a>";

        // No Stock
        $is_not_in_stock_url = add_query_arg( 'repricing_status', 'is_not_in_stock', $base_url );
        $class = ($current == 'is_not_in_stock' ? ' class="current"' :'');
        ...
        // FBA
        $is_fba_url = add_query_arg( 'repricing_status', 'is_fba', $base_url );
        $class = ($current == 'is_fba' ? ' class="current"' :'');
        $views['is_fba'] = "<a href='{$is_fba_url}' {$class} >".__('FBA','wpla')."</a>";

        // Non-FBA
        $is_not_fba_url = add_query_arg( 'repricing_status', 'is_not_fba', $base_url );
        $class = ($current == 'is_not_fba' ? ' class="current"' :'');
    ...
    function extra_tablenav( $which ) {
        ...
        $base_url = remove_query_arg( array( 'action' ) );
