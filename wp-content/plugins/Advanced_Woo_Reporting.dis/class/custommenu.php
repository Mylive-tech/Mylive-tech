								...
								<table class="display nodatatable" cellspacing="0" width="100%">
									...
									<?php $query = "
												SELECT 
													...
													GROUP_CONCAT(DISTINCT CONCAT(meta.meta_key,'|->|',meta.meta_value) ORDER BY meta.post_id  SEPARATOR '~|~') as gmx,
													posts.post_date as pdate
												FROM  {$wpdb->posts} AS posts
													LEFT JOIN {$wpdb->postmeta} AS meta ON(posts.ID = meta.post_id)
												WHERE 
													posts.post_status IN ( 'wc-completed','wc-processing','wc-on-hold','wc-pending','wc-cancelled','wc-failed','wc-refunded' )
													AND posts.post_type IN ( 'shop_order' )
													AND meta.meta_key IN ( '_billing_first_name','_billing_last_name','_billing_email','_order_total', '_order_discount', '_order_shipping', '_order_shipping_tax', '_order_tax')
									...
									<tbody>
										...
										echo("<tr>");
											...
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_total, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_discount, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_shipping, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_shipping_tax, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_tax, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)0, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_tax + $val -> gmx -> _order_shipping_tax, 2, '.', ',');
											echo("</td>");
											echo("<td>");
												 echo "$",number_format((float)$val -> gmx -> _order_total - $val -> gmx -> _order_discount + $val -> gmx -> _order_shipping + $val -> gmx -> _order_tax + $val -> gmx -> _order_shipping_tax, 2, '.', ',');
											echo("</td>");
										echo("</tr>");
										}
										?>
