...
class G1_Theme_Front {
    ...
    public function setup_comment_form() {
        add_filter( 'comment_form_default_fields', 'g1_comment_form_default_fields' );
        add_filter( 'comment_form_field_comment', 'g1_comment_form_field_comment' );
        add_filter( 'comment_form_defaults', 'g1_comment_form_defaults' );
    }
    ...
    function compose_css_config() {
            ...
            'rules' => array(
                ...
                'id2' => array(
                    'selectors' => array(
                        ...
                        '.g1-tabs--simple .g1-tabs-nav-item:before',
                        '.g1-tabs--simple .g1-tabs-nav-item:after',
                        '.entry-header:after',
        ...
        $config['cs_2_text1'] = array(
            ...
            'rules' => array(
                ...
                'id1a' => array(
                    'selectors' => array(
                        '.g1-searchbox #s::-webkit-input-placeholder',
                    ),
                    'css' => array(
                        'id1' => array( 'prop' => 'color', 'val' => '#%hex%' ),
                    ),
                ...
                'id1b' => array(
                    'selectors' => array(
                        '.g1-searchbox #s:-moz-placeholder',
                    ),
                    'css' => array(
                        'id1' => array( 'prop' => 'color', 'val' => '#%hex%' ),
                    ),
                ...
                'id1d' => array(
                    'selectors' => array(
                        '.g1-searchbox #s:-ms-input-placeholder',
                    ),
                    'css' => array(
                        'id1' => array( 'prop' => 'color', 'val' => '#%hex%' ),
                    ),
        ...
        $new_config = array();
        foreach ( G1_Theme()->get_theme_areas() as $theme_area => $theme_area_label ) {
            $ta_config = array();
            ...
            foreach ( $config as $setting_id => $setting_args ) {
                ...
                foreach ( $setting_args['rules'] as $rule_id => $rule_args ) {
                    ...
                    foreach ( $new_rule_args['selectors'] as $key => $selector ) {
                        $new_rule_args['selectors'][ $key ] = '#g1-' . $theme_area . ' ' . $selector;
                    }
...
//        $new_config['ta_header']['cs_2_text1']['rules']['id1']['selectors'][] = '#g1-primary-nav-menu li.g1-type-column > .g1-submenus a';
    ...
    public function capture_css_config() {
        ...
        foreach ( G1_Theme()->get_theme_areas() as $slug => $label ) {
    ...
    public function enqueue_styles() {
        ...
        // default, not cached version
        $g1_dynamic_style_filename = 'g1-dynamic-style.php';

        if ( defined( 'G1_DYNAMIC_STYLE_CACHE' ) && G1_DYNAMIC_STYLE_CACHE === true ) {
            $g1_dynamic_style_filename = 'g1-dynamic-style.css';
        }

        wp_register_style( 'g1_dynamic_style', $uri . 'css/'. $g1_dynamic_style_filename .'?respondjs=no', array('g1_screen'), $version, 'screen' );
    ...
    public function enqueue_scripts() {
        ...
        wp_register_script( 'caroufredsel', $parent_uri . 'js/carouFredSel/jquery.carouFredSel-6.2.0-packed.js', array('jquery'), $version, true );
        ...
        wp_register_script( 'skrollr', $parent_uri . 'js/skrollr/skrollr.min.js', array(), '1.0', true );
