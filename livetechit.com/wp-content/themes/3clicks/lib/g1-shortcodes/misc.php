...
class G1_Carousel_Shortcode extends G1_Shortcode {
    ...
    protected function do_shortcode() {
        ...
        $content = '<div class="g1-content"><ul class="g1-carousel-items">' . $content . '</ul></div>';
        ...
        if( !empty( $content ) && $rotate_items > 0 ) {
            $out .= '<div id="'.$final_id.'" class="'.sanitize_html_classes($final_class).'" data-g1-rotate-items="'. esc_attr($rotate_items) .'">';
            $out .= $content;
            $out .= $toolbar;
            $out .= '</div>';
        }
