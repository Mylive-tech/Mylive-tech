...
// Store variation data in meta
    ...
    function printing_receipt()
    {
        ...
        if(isset($_POST['order_id']) && $_POST['order_id']){
            ...
            require_once( WC_POS()->plugin_path().'/includes/views/html-print-receipt.php' );
        }
        die();
    }
    function load_pending_orders()
    {
        ...
        <div id="retrieve_sales_popup_content">
            ...
            $_GET['s'] = $term;
            $wc_pos_order_list = new WC_POS_Order_List();
            $wc_pos_order_list->prepare_items($id, $term);
    ...
    function load_order_data()
    {
        check_ajax_referer( 'load_order_data', 'security' );
            ...
            if(!empty($order_items) && is_array($order_items)){
                $show_stock = woocommerce_settings_get_option( 'wc_pos_show_stock', 'yes' );
                foreach ($order_items as $item_id => $item) {
                    $product = $order->get_product_from_item($item);
                    if($product){
                        $item['featured_src'] = wp_get_attachment_url( get_post_thumbnail_id( $product->is_type( 'variation' ) ? $product->variation_id : $product->id ) );
                        $item['barcode'] = $product->get_sku();
                        $item['taxable'] = $product->is_taxable();
                        $item['categories_ids'] = wp_get_post_terms( $product->id, 'product_cat', array( "fields" => "ids" ) );

                        $product->attributes = array_shift(woocommerce_get_product_terms($product->id, 'pa_koostis', 'names'));
                    }

                    $items[$item_id]['product'] = $product;
            ...
            $coupons_items = $order->get_items( 'coupon' );
            $coupons        = '';
            if ( $coupons_items ) {
                $coupons = array();
                foreach ( $coupons_items as $item_id => $item ) {
                    $coupons[$item['name']] = $item['item_meta']['discount_amount'];
                }
            }
            $loaded_info = array(
                'order_items'   => $items,
        ...
        echo '<!--WC_POS_START-->' . json_encode($loaded_info) . '<!--WC_POS_END-->';
        die();
    }
