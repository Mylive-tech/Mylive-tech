...
<body id="pos_receipt">
	...
	<div id="pos_receipt_info">
		<table class="order-info">
			<tbody>
				...
				<?php } ?>
				<?php if( $receipt_options['print_server'] == 'yes') { $current_user = wp_get_current_user(); ?>
				<tr>
					<th><?php echo $receipt_options['served_by_label']; ?></th>
					<td><?php if( $receipt_options['served_by_label']){?>
					<?php } ?> <?php echo $current_user->display_name; ?> on <?php echo $register_name; ?>
					</td>
				</tr>
				<?php  } ?>
			</tbody>
	                                    ...
	                                    if ( isset( $item['line_total'] ) ) {
	                                        if ( isset( $item['line_subtotal'] )  ) {
												...
												if($line_subtotal > $line_total){
	                                            	echo '<del>' . wc_price( $order->get_item_subtotal( $item, $tax_display, true ), array( 'currency' => $order->get_order_currency() ) ) . '</del> ';
	                                            }
	                                        }
                                        	$item_total = (float)$order->get_item_total( $item, $tax_display, true );
                                        	$item_qty   = (int)$item['qty'];
                                        	$_cart_subtotal += ($item_qty*$item_total);
	                                        echo wc_price( $order->get_item_total( $item, $tax_display, true ), array( 'currency' => $order->get_order_currency() ) );
	                                    }
	                                ?>
	                            </td>
	                            ...
	                            <?php
									if ( isset( $item['line_total'] ) ) {
										if ( isset( $item['line_subtotal'] ) ) {
											...
											$line_total    = (float)$item['line_total'];
											if($line_subtotal > $line_total){
												echo '<del>' . $order->get_formatted_line_subtotal( $item ) . '</del> ';
											}
										}
										...
										if ( 'excl' == $order->tax_display_cart ) {
											...
											echo wc_price( $order->get_line_total( $item ), array( 'ex_tax_label' => $ex_tax_label, 'currency' => $order->get_order_currency() ) );
										} else {
											echo  wc_price( $order->get_line_total( $item, true ), array('currency' => $order->get_order_currency()) );
										}
	                ...
	                if ( $totals = $order->get_order_item_totals() ) {
	                    ...
	                    foreach ( $totals as $total_key => $total ) {
	                        if( $total_key == 'cart_subtotal' ){
	                            ...
	                            $total['value'] =  wc_price( $_cart_subtotal, array( 'currency' => $order->get_order_currency() ) );
	                        }
	...
	<div id="pos_receipt_barcode">
	    <center>
	         <?php if($receipt_options['print_barcode'] == 'yes'){ ?><p id="print_barcode"><img src="<?php echo  WC_POS()->plugin_url(). '/includes/classes/barcode/image.php?filetype=PNG&dpi=72&scale=2&rotation=0&font_family=Arial.ttf&font_size=12&thickness=30&start=NULL&code=BCGcode128&text='.str_replace("#", "", $order->get_order_number()); ?>" alt=""></p>
	        <?php } ?>
	    </center>
	</div>
