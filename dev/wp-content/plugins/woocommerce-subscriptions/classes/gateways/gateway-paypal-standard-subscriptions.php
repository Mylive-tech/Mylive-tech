...
class WC_PayPal_Standard_Subscriptions {
	...
	public static function process_paypal_ipn_request( $transaction_details ) {
		...
		// If we have an express checkout operation and couldn't find and order ID, it must be becasue the operation didn't apply to a PayPal Standard Subscription, so we'll leave it for other extensions to handle
		if ( 'recurring_payment_suspended_due_to_max_failed_payment' == $transaction_details['txn_type'] && empty( $order_id ) ) {
			if ( self::$debug ) {
		...
		if ( self::$debug ) {
			$log_message = 'IPN subscription request processed for ' . $order_id;
			if ( isset( $ipn_id ) && ! empty( $ipn_id ) ) {
				$log_message .= sprintf( ' (%s)', $ipn_id );
			}
			self::$log->add( 'paypal', $log_message );
		}
	...
	private static function get_order_id_and_key( $args ) {
		...
		// Couldn't find the order ID by subscription ID, so it's either not set on the order yet or the $args doesn't have a subscr_id, either way, let's get it from the args
		if ( ! isset( $order_id ) ) {

			// If the IPN is for an express checkout operation, we only want to use the 'recurring_payment_id' stored on the order with the 'PayPal Subscriber ID' attempted above because we know for sure the events we want to handle (i.e. suspension) will only be send for existing subscriptions where this data should be stored and we don't want to handle an IPN for another plugin, like PayPal Digital Goods or PayPal Express Checkout
			if ( isset( $arg['rp_invoice_id'] ) ) {
				$order_id  = $order_key = '';
			} elseif ( is_numeric( $args['custom'] ) ) { // WC < 1.6.5
