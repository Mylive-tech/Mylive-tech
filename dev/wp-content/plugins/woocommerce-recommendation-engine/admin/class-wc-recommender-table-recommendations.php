...
class WC_Recommender_Table_Recommendations extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct( array('singular' => 'Product', 'plural' => 'Products', 'ajax' => false, 'screen' => 'wc_recommender_admin') );
	}
	...
	public function get_columns() {
		...
		$c = array(
		    ...
		    'actions' => __( 'Actions', 'wc_recommender' )
		);
				...
				foreach ( $columns as $column_name => $column_display_name ) {
					...
					switch ( $column_name ) {
						...
						case 'related':
							...
							break;
						case 'actions' :
							echo '<td><button rel="' . $rec->ID . '" type="button" class="button do_wc_recommender_build_recommendation wc-reload" title="' . __( 'Rebuild', 'wc_recommender' ), '"><span>' . __( 'Rebuild', 'wc_recommender' ) . '</span></button></td>';
							break;
					}
				}
	...
	public function prepare_items() {
		...
		if ( isset( $_REQUEST['s'] ) && !empty($_REQUEST['s']) ) {
			$where_sql .= $wpdb->prepare( $where_sql . ' AND LOWER( post_title ) LIKE %s', '%' . strtolower( like_escape( $_REQUEST['s'] ) ) . '%' );
		}
