...
class WC_FirstData_API {
	...
	private function parse_response( $response ) {
		...
		if ( $response->has_transaction_error() ) {
			throw new Exception( sprintf( __( 'Error: [%s] - %s', WC_FirstData::TEXT_DOMAIN ), $response->get_error_number(), $response->get_error_description() ) );
		}
...
class WC_FirstData_API_Request extends XMLWriter {
	...
	public function get_transaction_request_xml( $order ) {
		...
		$this->writeElement( 'Reference_No', ltrim( str_replace( array( '|', '^', '%', '\\', '/', '[', ']', '~', '`' ), '', $order->get_order_number() ), _x( '#', 'hash before order number', WC_FirstData::TEXT_DOMAIN ) ) );
	...
	public function get_refund_request_xml( $order ) {
		...
		// standard elements
		$this->writeElement( 'Reference_No',   SV_WC_Helper::str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before order number', WC_FirstData::TEXT_DOMAIN ) ) ) );
		$this->writeElement( 'Reference_3',    $order->id );                         // order reference number
		$this->writeElement( 'Client_IP',      $_SERVER['REMOTE_ADDR'] );            // fraud investigation
		$this->writeElement( 'Client_Email',   $order->billing_email );              // fraud investigation
		$this->writeElement( 'Currency',       $order->get_order_currency() );
...
class WC_FirstData_API_Response extends SimpleXMLElement {
	...
	public function get_exact_response_code() {
		...
		if ( ! isset( $this->EXact_Resp_Code ) )
			return __( 'N/A', WC_FirstData::TEXT_DOMAIN );
	...
	public function get_transaction_type() {
		...
		switch ( $transaction_type ) {
			...
			default: return __( 'N/A', WC_FirstData::TEXT_DOMAIN );
		}
	}
	...
	public function get_failure_message() {
		...
		if ( isset( $this->Bank_Resp_Code ) && (string) $this->Bank_Resp_Code ) {
			// usually null
			$resp_code_2 = ( (string) $this->Bank_Resp_Code_2 ) ? ' ' . sprintf( __( '[Code2 %s]', WC_FirstData::TEXT_DOMAIN ), (string) $this->Bank_Resp_Code_2 ) : '';

			return sprintf( __( 'Bank Response [Code %s]%s Message: %s', WC_FirstData::TEXT_DOMAIN ), (string) $this->Bank_Resp_Code, $resp_code_2, (string) $this->Bank_Message );

		} elseif ( isset( $this->Error_Number ) && (string) $this->Error_Number ) {
			return sprintf( __( 'Error Response [Code %s] Description: %s', WC_FirstData::TEXT_DOMAIN ), (string) $this->Error_Number, (string) $this->Error_Description );
		}

		return __( 'Unknown error', WC_FirstData::TEXT_DOMAIN );
	}
