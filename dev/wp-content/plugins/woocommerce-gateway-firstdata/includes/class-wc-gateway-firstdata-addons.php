...
class WC_Gateway_FirstData_Addons extends WC_Gateway_FirstData {
		...
		if ( wc_firstdata()->is_subscriptions_active() && ( SV_WC_Plugin_Compatibility::is_wc_subscriptions_version_gte_2_0() ? wcs_order_contains_subscription( $order_id ) : WC_Subscriptions_Order::order_contains_subscription( $order_id ) ) ) {
			...
			try {
				...
				if (  0 == $order->payment_total || $this->do_transaction( $order ) ) {
					...
					if ( empty( $token ) ) {
						$order->update_status( 'on-hold', __( 'Expected token not returned for transaction, verify Transarmor Token is properly configured in your First Data account.', WC_FirstData::TEXT_DOMAIN ) );
					}
	...
	public function process_subscription_renewal_payment( $amount_to_charge, $order, $product_id = null ) {
		...
		try {
			...
			$order->description      = sprintf( __( '%s - Renewal for Subscription Order %s', WC_FirstData::TEXT_DOMAIN ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $order->get_order_number() );
			...
			$failed_message = sprintf( __( 'First Data Subscription Renewal Payment Failed (Result: %s)', WC_FirstData::TEXT_DOMAIN ), $e->getMessage() );
	...
	public function maybe_render_subscription_payment_method( $payment_method_to_display, $subscription ) {
		...
		if ( $card ) {
			$payment_method_to_display = sprintf( __( 'Via %s ending in %s', WC_FirstData::TEXT_DOMAIN ), $card['type'], $card['last_four'] );
		}
		...
		$meta[ $this->id ] = array(
			'post_meta' => array(
				'_wc_firstdata_transarmor_token'   => array(
					...
					'label' => __( 'TransArmor Token', WC_FirstData::TEXT_DOMAIN ),
				),
			)
		);
	...
	public function subscriptions_admin_validate_payment_meta( $meta ) {
		...
		if ( empty( $meta['post_meta']['_wc_firstdata_transarmor_token']['value'] ) ) {
			throw new Exception( __( 'TransArmor Token is required.', WC_FirstData::TEXT_DOMAIN ) );
		} elseif ( ! ctype_digit( (string) $meta['post_meta']['_wc_firstdata_transarmor_token']['value'] ) ) {
			throw new Exception( __( 'TransArmor Token must be numeric.', WC_FirstData::TEXT_DOMAIN ) );
		}
	}
