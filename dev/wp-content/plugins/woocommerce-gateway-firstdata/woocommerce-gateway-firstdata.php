...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.1.1', __( 'WooCommerce First Data Gateway', 'woocommerce-gateway-firstdata' ), __FILE__, 'init_woocommerce_gateway_first_data', array( 'minimum_wc_version' => '2.2', 'backwards_compatible' => '4.0.0' ) );
...
class WC_FirstData extends SV_WC_Plugin {
		...
		if ( $this->using_gge4_gateway() ) {
			$actions['use_global_gateway'] = sprintf(
				'<a href="%s" title="%s">%s</a>',
					...
					esc_attr__( 'Use the Global Gateway', self::TEXT_DOMAIN ),
					__( 'Use Global Gateway', self::TEXT_DOMAIN )
				);
		} else {
				...
				'<a href="%s" title="%s">%s</a>',
					esc_url( wp_nonce_url(
						add_query_arg(
							...
							'admin.php' ),
						'wc-firstdata-change-gateway_' . $this->get_file() ) ),
					esc_attr__( 'Use the Payeezy gateway', self::TEXT_DOMAIN ),
					__( 'Use Payeezy', self::TEXT_DOMAIN )
			);
		}
	...
	private function add_gateway_switch_admin_notice() {
		...
		if ( isset( $_GET['wc_firstdata_gateway'] ) && $_GET['wc_firstdata_gateway'] ) {
			if ( self::GGE4_GATEWAY_CLASS_NAME == $_GET['wc_firstdata_gateway'] ) {
				$message = __( "First Data Payeezy Gateway is now being used.", self::TEXT_DOMAIN );
			} elseif ( self::GLOBAL_GATEWAY_CLASS_NAME == $_GET['wc_firstdata_gateway'] ) {
				$message = __( "First Data Global Gateway is now being used.", self::TEXT_DOMAIN );
			}
		...
		if ( isset( $settings['enabled'] ) && 'yes' == $settings['enabled'] ) {
			...
			if ( isset( $settings['environment'] ) && 'production' == $settings['environment'] ) {
				...
				if ( 'no' === get_option( 'woocommerce_force_ssl_checkout' ) ) {
					$message = sprintf(
						__( "%sFirst Data Error%s: WooCommerce is not being forced over SSL; your customer's credit card data is at risk.", self::TEXT_DOMAIN ),
						'<strong>', '</strong>'
					);
			...
			if ( isset( $settings[ $environment_prefix . 'gateway_id' ] ) && $settings[ $environment_prefix . 'gateway_id' ] &&
				...
				$message = sprintf(
					__( "%sFirst Data Payeezy Gateway:%s It's recommended that you %sconfigure%s the new Key ID and Hmac Key settings for increased transaction security.  See the %sdocumentation%s for further details.", self::TEXT_DOMAIN ),
					'<strong>', '</strong>',
	...
	public function add_customer_data( $user ) {
		...
		<table class="form-table">
			<tr>
				<th><?php _e( 'Payment Tokens', $this->text_domain ); ?></th>
				<td>
					<?php
					if ( empty( $payment_tokens ) ):
						...
						<ul style="margin:0;">
							...
							foreach ( $payment_tokens as $token => $payment ) :
								...
								?>
									<li>
										<?php echo $token; ?> (<?php _e( sprintf( '%s ending in %s expiring %s', $payment['type'], $payment['last_four'], substr( $payment['exp_date'], 0, 2 ) . '/' . substr( $payment['exp_date'], -2 ) ) ); echo ( $payment['active'] ? ' <strong>' . __( 'Default card', $this->text_domain ) . '</strong>' : '' ); ?>)
										<a href="#" class="wc-firstdata-payment-token-delete" data-payment_token="<?php echo $token; ?>"><?php _e( 'Delete', $this->text_domain ); ?></a>
									</li>
								<?php
			...
			<tr>
				<th><?php _e( 'Add a Payment Token', $this->text_domain ); ?></th>
				<td>
					<input type="text" id="wc-firstdata-payment-token" name="wc_firstdata_payment_token" placeholder="<?php esc_attr_e( 'Token', $this->text_domain ); ?>" style="width:145px;" />
					...
					<input type="text" id="wc-firstdata-payment-token-exp-date" name="wc_firstdata_payment_token_exp_date" placeholder="<?php esc_attr_e( 'Expiry Date (01/15)', $this->text_domain ); ?>" style="width:140px;" />
					<br/>
					<span class="description"><?php _e( 'Payment tokens can be added with the information found in the transaction list in your First Data account, so long as your TransArmor token was configured at the time of the transaction.  For "Token" use the "Card Number" for the tokenized transaction, found in your First Data account transaction list.', $this->text_domain ); ?></span>
				</td>
			</tr>
		</table>
		<?php
	...
	public function get_plugin_name() {
		return __( 'WooCommerce First Data', self::TEXT_DOMAIN );
	}
...
/**
 * The WC_FirstData global object, exists only for backwards compat
 *
 * @deprecated 3.6.0
 * @name $wc_firstdata
 * @global WC_FirstData $GLOBALS['wc_firstdata']
 */
$GLOBALS['wc_firstdata'] = wc_firstdata();
