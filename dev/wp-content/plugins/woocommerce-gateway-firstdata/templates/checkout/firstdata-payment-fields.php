...
<fieldset>
	...
	if ( $tokens ) : ?>
		<p class="form-row form-row-wide">
			<a class="button" style="float:right;" href="<?php echo esc_url( SV_WC_Plugin_Compatibility::wc_get_page_permalink( 'woocommerce_myaccount_page_id' ) ); ?>#woocommerce-firstdata-my-payment-methods"><?php echo wp_kses_post( apply_filters( 'wc_firstdata_manage_my_cards', __( "Manage My Cards", WC_FirstData::TEXT_DOMAIN ) ) ); ?></a>
			<?php foreach( $tokens as $token => $card ) : ?>
				...
				<label style="display:inline;" for="firstdata-token-<?php echo esc_attr( $token ); ?>"><?php printf( __( '%s ending in %s (expires %s)', WC_FirstData::TEXT_DOMAIN ), esc_html( $card['type'] ), esc_html( $card['last_four'] ), esc_html( substr( $card['exp_date'], 0, 2 ) . '/' . substr( $card['exp_date'], -2 ) ) ); ?></label><br />
			<?php endforeach; ?>
			<input type="radio" id="firstdata-use-new-card" name="firstdata-token" style="width:auto;" value="" /> <label style="display:inline;" for="firstdata-use-new-card"><?php echo __( 'Use a new credit card', WC_FirstData::TEXT_DOMAIN ); ?></label>
		</p>
		<div class="clear"></div>
	<?php endif; ?>
	...
	<div class="firstdata-new-card-form" <?php echo ( $tokens ? 'style="display:none;"' : '' ); ?>>
		...
		<p class="form-row form-row-last">
			...
			<select name="firstdata-exp-month" id="firstdata-exp-month" class="woocommerce-select woocommerce-cc-month" style="width:auto;">
				<option value=""><?php _e( 'Month', WC_FirstData::TEXT_DOMAIN ) ?></option>
			...
			<select name="firstdata-exp-year" id="firstdata-exp-year" class="woocommerce-select woocommerce-cc-year" style="width:auto;">
				<option value=""><?php _e( 'Year', WC_FirstData::TEXT_DOMAIN ) ?></option>
		...
		if ( $tokenization_allowed || $tokenization_forced ) :
			if ( $tokenization_forced ) :
				...
				<p class="form-row">
					...
					<label for="firstdata-tokenize-card" style="display:inline;"><?php echo wp_kses_post( apply_filters( 'wc_firstdata_tokenize_card_text', __( "Securely Save Card to Account", WC_FirstData::TEXT_DOMAIN ) ) ); ?></label>
				</p>
				<div class="clear"></div>
				<?php
			endif;
