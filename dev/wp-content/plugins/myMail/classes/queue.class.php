...
class mymail_queue {
	...
	public function init() {
		...
		add_action('mymail_cron_worker', array( &$this, 'autoresponder_usertime'), 30);
	...
	public function autoresponder_usertime_OLD($campaign_id = NULL) {
		...
		foreach($campaigns as $campaign){
			...
			foreach($chunks as $ids){
				...
				foreach($subscribers as $subscriber){
					...
					$nextdate = strtotime($subscriber->date) + $send_offset;
	...
	public function autoresponder_usertime($campaign_id = NULL, $force = false) {
		...
		foreach($campaigns as $campaign){
			...
			$sql = str_replace(
				'SELECT a.ID',
				"SELECT a.ID, x.meta_value AS date",
				$sql);

			$sql = str_replace(
				'WHERE 1',
				"LEFT JOIN {$wpdb->prefix}mymail_subscriber_fields AS x ON a.ID = x.subscriber_id WHERE 1".$specialcond,
				$sql);

			$sql = str_replace(
				'WHERE 1',
				$wpdb->prepare("WHERE 1 AND x.meta_key = %s AND STR_TO_DATE(x.meta_value, %s) <= %s", $autoresponder_meta['uservalue'], '%Y-%m-%d', date('Y-m-d', $offsettimestamp)),
				$sql);

			$sql = str_replace(
				'ORDER BY a.ID',
				'ORDER BY x.meta_value',
				$sql);
	...
	public function progress($cron_used = false) {
		...
		$this->cron_log('Local Time', '<strong>' . date('Y-m-d H:i:s', time()+$timeoffset).' - '.(time()+$timeoffset) . '</strong>');
