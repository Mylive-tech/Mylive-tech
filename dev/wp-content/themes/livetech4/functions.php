...
wp_enqueue_script( 'divi-fitvids', $template_dir . '/js/jquery.fitvids-min.js', array( 'jquery' ), $theme_version, true );
...
wp_register_script( 'easypiechart', $template_dir . '/js/jquery.easypiechart-min.js', array( 'jquery' ), $theme_version, true );
...
wp_enqueue_script( 'smooth-scroll', $template_dir . '/js/smoothscroll-min.js', array( 'jquery' ), $theme_version, true );
...
function et_pb_portfolio_meta_box() { ?>
  <div class="et_project_meta"> <strong class="et_project_meta_title"><?php echo esc_html__( 'Skills', 'Divi' ); ?></strong>
...
: 'light'; ?>
  <p class="et_pb_page_settings et_pb_page_layout_settings">
    <label for="et_pb_page_layout" style="display: block; font-weight: bold; margin-bottom: 5px;">
      <?php esc_html_e( 'Page Layout', 'Divi' ); ?>
      : </label>
...
selected( $layout_value, $page_layout )
  ...
  <p class="et_pb_page_settings et_pb_side_nav_settings" style="display: none;">
    <label for="et_pb_side_nav" style="display: block; font-weight: bold; margin-bottom: 5px;">
      <?php esc_html_e( 'Side Navigation', 'Divi' ); ?>
      : </label>
  ...
  <p class="et_divi_quote_settings et_divi_audio_settings et_divi_link_settings et_divi_format_setting">
    <label for="et_post_use_bg_color" style="display: block; font-weight: bold; margin-bottom: 5px;">
      ...
      <?php esc_html_e( 'Use Background Color', 'Divi' ); ?>
    </label>
  </p>
  <p class="et_post_bg_color_setting et_divi_format_setting">
    <label for="et_post_bg_color" style="display: block; font-weight: bold; margin-bottom: 5px;">
      <?php esc_html_e( 'Background Color', 'Divi' ); ?>
      : </label>
  ...
  <p class="et_divi_quote_settings et_divi_audio_settings et_divi_link_settings et_divi_format_setting">
    <label for="et_post_bg_layout" style="font-weight: bold; margin-bottom: 5px;">
      <?php esc_html_e( 'Text Color', 'Divi' ); ?>
      : </label>
...
.woocommerce a.button.alt, .woocommerce-page a.button.alt, .woocommerce button.button.alt, .woocommerce-page button.button.alt, .woocommerce input.button.alt, .woocommerce-page input.button.alt, .woocommerce #respond input#submit.alt, .woocommerce-page #respond input#submit.alt, .woocommerce #content input.button.alt, .woocommerce-page #content input.button.alt, .woocommerce a.button, .woocommerce-page a.button, .woocommerce button.button, .woocommerce-page button.button, .woocommerce input.button, .woocommerce-page input.button, .woocommerce #respond input#submit, .woocommerce-page #respond input#submit, .woocommerce #content input.button, .woocommerce-page #content input.button, .woocommerce-message, .woocommerce-error, .woocommerce-info { background: <?php echo esc_html( et_get_option( 'accent_color', '#2EA3F2' ) ); ?> ; }
...
remove_action('woocommerce_after_single_product_summary', 'ask_about_product');



add_action('woocommerce_after_single_product_summary', 'ask_about_product',3);**/
...
/** Move Wishlist question **/



remove_action('woocommerce_single_product_summary', 'yith_wcwl_after_wishlist');
...
/* Upsells to show 4 columns */



function woocommerce_upsell_display( $posts_per_page = 4, $columns = 4, $orderby = 'price' ) {
...
wp_redirect( 'https://www.mylive-tech.com/my-account/' );
...
return 'https://www.mylive-tech.com/store/';
...
$existing_mimes['dmg'] = 'application/octet-stream';
...
wp_register_script('jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/1.8.0/jquery.min.js', false, null);
...
wp_register_script('client-js', get_template_directory_uri() . '/js/jquery.flexisel-min.js', false, null, true);
...
/* Product Enquiry Form Tab */



add_filter( 'woocommerce_product_tabs', 'sb_woo_product_enquiry_tab'); //sb_woo_new_test_tab



function sb_woo_product_enquiry_tab_content() {



echo  '<div id="product-enquiry">';



echo '<h2>Ask A Question About This Product</h2>';



echo do_shortcode('[vfb id=9]');



echo '<p>You can also call us toll-free on: 1 (888) 361-8511</p>';
...
function sb_woo_product_enquiry_tab($tabs) {
...
'callback' => 'sb_woo_product_enquiry_tab_content'
...
function add_login_logout_link( $args  ) {
...
$loginoutlink = wp_loginout('my-account', false);
...
/** Change login page text to username or email **/
...
/** Grid List view immediate effect **/
...
echo "<div class=\"lt-pagination\"><span>Page ".$paged." of ".$pages."</span>";
...
/** load fontawesome from jsdelivr **/



//wp_enqueue_style( 'lt-fontawesome', 'http://cdn.jsdelivr.net/fontawesome/4.2.0/css/font-awesome-ie7.css', array(), null );



//wp_enqueue_style( 'lt-fontawesome1', 'http://cdn.jsdelivr.net/fontawesome/4.2.0/css/font-awesome.css', array(), null );
...
add_action( 'init', 'lt_breadcrumbs' );
...
add_action( 'woocommerce_before_main_content','lt_yoast_breadcrumb', 20, 0);

function lt_yoast_breadcrumb() {
...
//** Registration Form - unset website URL  **/
...
$message = sprintf( "Hello %s,", affiliate_wp()->affiliates->get_affiliate_name( $affiliate_id ) ) . "\n\n";



$message .= sprintf( "Thank you for applying to become an affiliate on %s. Your application will be reviewed shortly.", get_bloginfo( 'name' ) ) . "\n\n"; 
...
$email = array( get_option( 'admin_email' ), 'webmaster@mylive-tech.com', 'frank@mylive-tech.com', 'tush@mylive-tech.com' );
...
return __( 'Sign Up Today', 'woocommerce' );
...
$string = '<a href="' . get_permalink( $post ) . '" rel="'.$rel.'" target="_self">';
...
/** Per category social image **/

add_filter('wpseo_opengraph_image', 'category_image');
function category_image($image) {
   global $post;
   if( is_product_category ( 'networking-rackmount', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/networking-rackmount-og.jpg';
   }
   if( is_product_category ( 'computers-gadgets-and-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/computers-gadgets-and-hardware-og.png';
   }
    if( is_product_category ( 'home-solutions', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/home-solutions-main.png';
   }
   if( is_product_category ( 'business-solutions', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/business-solutions-main.png';
   }
   if( is_product_category ( 'live-tech-support-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/technical-support-services.png';
   }
   if( is_product_category ( 'live-tech-hosted-cloud-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-cloud-services-main.png';
   }
   if( is_product_category ( 'live-tech-hosted-exchange', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-exchange.png';
   }
   if( is_product_category ( 'live-tech-hosted-sharepoint-live-tech-hosted-cloud-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-sharepoint.png';
   }
    if( is_product_category ( 'live-tech-skyworker', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-skyworker.png';
   }
     if( is_product_category ( 'live-tech-hosted-lync', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-lync.png';
   }
    if( is_product_category ( 'live-tech-skynox-cloud-backup', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-skynox.png';
   }
    if( is_product_category ( 'live-techs-cloud-based-web-hosting', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-hosting.png';
   }
    if( is_product_category ( 'website-development-and-hosting-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-webdev.png';
   }
   if( is_product_category ( 'affordable-logo-designing-corporate-identity-designing', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-logo-design.png';
   }
   if( is_product_category ( 'lynk-voip-phone-service', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-lynk-voip.png';
   }
    if( is_product_category ( 'home-voip-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-home-voip.png';
   }
    if( is_product_category ( 'business-voip-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-business-voip.png';
   }
   if( is_product_category ( 'pre-packaged-fortivoice-voip-plans', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-pre-forti.png';
   }
   if( is_product_category ( 'fortivoice-phone-systems', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-forti-phone.png';
   }
    if( is_product_category ( 'high-volume-calling-plans', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-high-volume-voip.png';
   }
    if( is_product_category ( 'fortivoice-phone-systems-computers-gadgets-and-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-forti-phone.png';
   }
    if( is_product_category ( 'networking', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-networking.png';
   }
    if( is_product_category ( 'security-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-security-hardware.png';
   }
   if( is_product_category ( 'computer-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-computer-hardware-acc.png';
   }
    if( is_product_category ( 'live-tech-custom-computer', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-custom-computer.png';
   }
    if( is_product_category ( 'mobile-hardware-accessories', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-mobile-hardware.png';
   }
    if( is_product_category ( 'server-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-server-hardware.png';
   }
    if( is_product_category ( 'software', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-software.png';
   }
   if( is_product_category ( 'productivity-software', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-productivity.png';
   }
    if( is_product_category ( 'system-care-and-protection', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-system-care.png';
   }
   if( is_product_category ( 'vpn', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/livetech-vpn.png';
   }
   if( is_product_category ( 'corel', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/corel.png';
   }
   if( is_product_category ( 'microsoft', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/microsoft.png';
   }
    if( is_product_category ( 'kaspersky', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/kaspersky.png';
   }
    if( is_product_category ( 'bitdefender', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/bitdef.png';
   }
    if( is_product_category ( 'nuance', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/nuance.png';
   }
   if( is_product_category ( 'microsoft', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/window.png';
   }
    if( is_product_category ( 'stopzilla', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/stopzilla.png';
   }
   if( is_product_category ( 'trendmicro', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/Trend-Micro.png';
   }
    if( is_product_category ( 'packetviper', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/PacketViper.png';
   }
    if( is_product_category ( 'check-point', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/checkpoint.png';
   }
   if( is_product_category ( 'fortinet', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/rackmount.png';
   }
   
   if( is_product_category ( 'adobe', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/adobe.png';
   }
    if( is_product_category ( 'bluebeam', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/bluebeam.png';
   }
    if( is_product_category ( 'piriform', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/piriform.png';
   }
   if( is_product_category ( 'norton', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/norton.png';
   }
   if( is_product_category ( 'snom', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/snom.png';
   }
   if( is_product_category ( 'fortifone-ip-phones-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortifone-ip-phones-hardware';
   }
   if( is_product_category ( 'cel-fi', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/cel-fi.png';
   }
   if( is_product_category ( 'sonicwall', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/sonicwall.png';
   }
   if( is_product_category ( 'kerio', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/kerio.png';
   }
   if( is_product_category ( 'cisco-networking', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/cisco.png';
   }
   if( is_product_category ( 'watchguard', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/watchguard.png';
   }
   if( is_product_category ( 'computer-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/computer-hardware.png';
   }
   if( is_product_category ( 'fortivoice-starter-bundles-fortivoice-phone-systems-computers-gadgets-and-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortivoice-phone-systems.png';
   }
   
   if( is_product_category ( 'fortifone-cordless-ip-phones', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortifone-cordless-ip-phones.png';
   }
   if( is_product_category ( 'fortifone-analog-phone-fortivoice-phone-systems-computers-gadgets-and-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortifone-analog-phone.png';
   }
   if( is_product_category ( 'softphones-add-ons-fortivoice-phone-systems', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/softphones.png';
   }
   if( is_product_category ( 'android-tablets', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/android-tablets.png';
   }
   if( is_product_category ( 'fortivoice-pbx-units', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortivoice-pbx-units.png';
   }
   if( is_product_category ( 'fortifone-ip-phones', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortifone-ip-phones.png';
   }
   if( is_product_category ( 'fortivoice-starter-bundles', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortivoice-starter-bundles.png';
   }
   if( is_product_category ( 'ip-phones', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/ip-phones.png';
   }
   if( is_product_category ( 'operating-systems', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/operting-system.png';
   }
   if( is_product_category ( 'rackmount', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/rackmount.png';
   }
    if( is_product_category ( 'rackmount-kits-and-adaptors', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/rackmount-kits-and-adaptors.png';
   }
    if( is_product_category ( 'android-phones', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/android-phones.png';
   }
   if( is_product_category ( 'mobile-hardware-accessories', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/mobile-hardware-accessories.png';
   }
   if( is_product_category ( 'server-hardware', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/server-hardware.png';
   }
   if( is_product_category ( 'mobile', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/mobile.png';
   }
   if( is_product_category ( 'multimedia', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/multimedia.png';
   }
   if( is_product_category ( 'yearly-support-plans', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/yearly-support-plans.png';
   }
   if( is_product_category ( 'monthly-support-plans', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/monthly-support-plans.png';
   }
   if( is_product_category ( 'monthly-support-plans-bis-support-for-business', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/monthly-support-plans-bis-support-for-business.png';
   }
   if( is_product_category ( 'fortivoice-systems-tech-support', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/fortivoice-support-plans.png';
   }
   if( is_product_category ( 'yearly-support-plans-bis-support-for-business', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/yearly-support-plans-bis-support-for-business.png';
   }
   if( is_product_category ( 'live-tech-hosted-exchange-2010', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/live-tech-hosted-exchange-2010.png';
   }
   if( is_product_category ( 'live-tech-hosted-exchange-2013', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/live-tech-hosted-exchange-2013.png';
   }
   if( is_product_category ( 'website-backup', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/website-backup.png';
   }
   if( is_product_category ( 'ssl-certificates', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/ssl-certificates.png';
   }
    if( is_product_category ( 'design-services', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/design-services.png';
   }
   if( is_product_category ( 'business-starter-sites', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/business-starter-sites.png';
   }
   if( is_product_category ( 'hourly-code-packages', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/hourly-code-packages.png';
   }
   if( is_product_category ( 'flat-rate-support-bis-support-for-business', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/flat-rate-support-bis-support-for-business.png';
   }
   if( is_product_category ( 'onsite-services-bis-support-for-business', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/onsite-services-bis-support-for-business.png';
   }
   if( is_product_category ( 'quick-fix-per-minute-support-bis-support-for-business', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/quick-fix-per-minute-support-bis-support-for-business.png';
   }
   if( is_product_category ( 'office-365', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/office-365.png';
   }
   if( is_product_category ( 'shared-hosting', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/shared-hosting.png';
   }
   if( is_product_category ( 'microsoft-office-365', $post->ID ) ) {
       $image = get_stylesheet_directory_uri().'/images/social/microsoft-office-365.png';
   }
...
/** Add SKU below title **/
function wc_add_sku_title() {
global $product;  ?>
<div class="lt-sku"> SKU: <?php echo $product->get_sku(); ?> </div>
<?php
}
add_action( 'woocommerce_single_product_summary', 'wc_add_sku_title', 6 );
...
printf( '<span class="et_shop_image">%1$s<span class="et_overlay"><i class="fa fa-eye fa- "></i></span></span>',
...
if ( '' !== $thumb_src ) : ?>
    <div class="et_pb_portfolio_image <?php esc_attr_e( $orientation ); ?>"> <a href="<?php the_permalink(); ?>"> <img src="<?php esc_attr_e( $thumb_src); ?>" alt="<?php esc_attr_e( get_the_title() ); ?>"/>
      <div class="meta"> <span class="et_overlay"></span>
        <?php if ( 'on' === $show_title ) : ?>
        <h3>
          <?php the_title(); ?>
        </h3>
...
'module_id' => '',
  ...
  '<div%3$s class="et_pb_pricing clearfix%2$s%4$s"> %1$s
  </div>
  ',
  ...
  <div class="et_pb_pricing_table%1$s">
    <div class="et_pb_pricing_heading"> %2$s
      
      
      
      %3$s </div>
    <!-- .et_pb_pricing_heading -->
    <div class="et_pb_pricing_content_top"> <span class="et_pb_et_price">%6$s%7$s%8$s</span> </div>
  ...
  ( '' !== $title ? sprintf( '
  <h2 class="et_pb_pricing_title">%1$s</h2>
  ', esc_html( $title ) ) : '' ),
...
<p>%1$s</p>
', esc_html__( 'Make sure you entered the captcha.', 'Divi' ) );
...
$et_error_message .= sprintf( '
<p>%1$s</p>
', esc_html__( 'You entered the wrong number in captcha.', 'Divi' ) );
...
$et_error_message .= sprintf( '
<p>%1$s</p>
', esc_html__( 'Make sure you fill all fields.', 'Divi' ) );
...
$et_error_message = sprintf( '
<p>%1$s</p>
', esc_html__( 'Thanks for contacting us', 'Divi' ) );
...
<div class="et_pb_contact_right">
  <p class="clearfix"> %1$s =
    <input type="text" size="2" class="input et_pb_contact_captcha" value="" name="et_pb_contact_captcha">
  </p>
</div>
<!-- .et_pb_contact_right -->
',
...
$output = sprintf( '
<div id="%4$s" class="et_pb_contact_form_container clearfix%5$s"> %1$s
...
( '' !== $title ? sprintf( '
<h1 class="et_pb_contact_main_title">%1$s</h1>
', esc_html( $title ) ) : '' ),



( '' !== $et_error_message ? sprintf( '
<div class="et-pb-contact-message">%1$s</div>
', $et_error_message ) : '' ),
...
'<div%2$s class="et_pb_shop%3$s"> %1$s
</div>
',
...
( $subhead ? sprintf( '
<p class="et_pb_fullwidth_header_subhead">%1$s</p>
', $subhead ) : '' ),
...
<nav id="top-menu-nav">';
  ...
  ? sprintf( '<li%1$s><a href="%2$s">%3$s</a>
  </li>
  ',
...
<div class="et_pb_row clearfix"> %1$s
...
'
<div class="et_pb_map_pin" data-lat="%1$s" data-lng="%2$s" data-title="%3$s"> %4$s </div>
',
...
return $output;
	...
	//add_image_size( 'medium-thumbnail', 85, 85 );
	//add_image_size( 'single-product', 260, 9999 );
	
	
	add_filter( 'avatar_defaults', 'newgravatar' );

    function newgravatar ($avatar_defaults) {
    $myavatar = get_bloginfo('template_directory') . '/images/livetech-logo1.png';
	//echo $myavatar;
    $avatar_defaults[$myavatar] = "Mylive-Tech";
