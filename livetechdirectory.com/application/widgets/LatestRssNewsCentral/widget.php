...
class Widget_LatestRssNewsCentral extends Phpld_Widget {
	...
	function getContent() {
		...
		$sql = "SELECT * FROM `{$tables['news']['name']}`
				WHERE 1 {$where}
					ORDER BY DATE_ADDED DESC LIMIT 0," . $set['LIMIT'];
					
		if ($set['CACHE_RESULTS'] == "Yes") {
			$latestNews = $db->CacheGetAll($sql);
		} else {
			$latestNews = $db->GetAll($sql);
		}


		$this->tpl->assign("TITLE", $set['TITLE']);
		$this->tpl->assign("NEWS", $latestNews);
