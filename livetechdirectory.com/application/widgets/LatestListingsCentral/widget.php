...
/**
  ...
  # ################################################################################
 */
class Widget_LatestListingsCentral extends Phpld_Widget_LinksList {
    ...
    function getLinks() {
        ...
        $sql = "SELECT *, PLD_LINK.DESCRIPTION AS DESCRIPTION,
                                                    PLD_LINK.ID as ID FROM `{$tables['link']['name']}`
			                            INNER JOIN PLD_LINK_TYPE ON PLD_LINK_TYPE.ID = PLD_LINK.LINK_TYPE
										WHERE (PLD_LINK.`STATUS` = '2' OR {$permissions['permission_links_arts']}) {$email_conf} {$listingTypeWhere}
