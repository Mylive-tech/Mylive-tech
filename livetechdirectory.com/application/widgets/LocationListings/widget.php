...
# ################################################################################
 */
class Widget_LocationListings extends Phpld_Widget_LinksList {

    function getContent() {
        $set = $this->getFrontSettings();

        $location = $_GET['location'];
        $this->tpl->assign("TITLE", $location);
    ...
    public function getLinks()
    {
        ...
        $category = $categoryModel->getCategoryByUri($category);

        $links = $linkModel->getByLocation($location[1], $location[0], $category['ID']);

        $paginator->assign($this->tpl, $links->countWithoutLimit());
        return $links;
    }
}
