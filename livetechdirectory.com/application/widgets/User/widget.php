...
/**
 ...
 # ################################################################################
 */
 class Widget_User extends Phpld_Widget {
	...
	function getContent() {
		...
		$regular_user_details = $db->GetRow("SELECT `ID`, `LOGIN`, `NAME`, `EMAIL` FROM `{$tables['user']['name']}` WHERE `ID` = ".$db->qstr(Model_CurrentUser::getInstance()->getId())." AND `ACTIVE` = '1'");
$this->tpl->assign('regular_user_details', $regular_user_details);
