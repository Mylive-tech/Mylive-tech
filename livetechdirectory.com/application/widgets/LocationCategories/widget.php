...
# ################################################################################
 */
class Widget_LocationCategories extends Phpld_Widget {

    function getContent() {
        $location = $_GET['location'];
        $location = explode(',',$location);
        $location = new Model_Location_Entity(
            array(
                'STATE' => $location[1],
                'CITY'  => $location[0]
            )
        ...
        $categories = $categoriesModel->getByLocation($location['STATE'], $location['CITY']);

        $this->tpl->assign("TITLE", $set['TITLE']);
        $this->tpl->assign('categs', $categories);
