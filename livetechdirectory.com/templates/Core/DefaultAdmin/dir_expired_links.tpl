...
<div class="block">
   <!-- Links Search Form -->
<!--
   <form action="{$smarty.const.DOC_ROOT}/dir_expired_links.php" method="get">
...
<div class="block">
   <!-- Links List -->
   <form action="{$smarty.const.DOC_ROOT}/dir_expired_links.php" method="post" id="multiselect_list" name="multiselect_list">
      ...
      <thead>
         <tr>
            ...
            {foreach from=$columns key=col item=name}
               {if $col ne 'TITLE_URL'}
                  <th class="listHeader" id="{$col}"><a href="{$smarty.const.DOC_ROOT}/dir_expired_links.php{if isset($columnURLs) and is_array($columnURLs)}?{$columnURLs.$col}{/if}" title="{l}Sort by{/l}: {$name|escape|trim}" class="sort{if $SORT_FIELD eq $col and $requestOrder eq 1} {$smarty.const.SORT_ORDER}{/if}">{$name|escape|trim}</a></th>
               {/if}
            {/foreach}
      ...
      {foreach from=$list item=row key=id}
         <tr id="link-row-{$id}" class="{cycle values="odd,even"}">
            ...
            <td class="noborder">
            	<a href="{$smarty.const.DOC_ROOT}/admin_gallery.php?id={$id}" class="action photo"><span>{l}Gallery{/l}</span></a>
            </td>
            <td class="noborder last-child">
            	<a href="{$smarty.const.DOC_ROOT}/admin_products.php?id={$id}" class="action product"><span>{l}Products{/l}</span></a>
            </td>
            {/if}
         </tr>
         {foreachelse}
