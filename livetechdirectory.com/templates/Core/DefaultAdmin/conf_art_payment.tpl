...
<div class="block">
   <!-- Send Invoices -->
   <form method="post" name="send_invoices" id="send_invoices" action="{$smarty.const.DOC_ROOT}/article_payment.php?action=I:0:0">
      <table class="list">
      <thead>
         <tr>
   ...
   <table class="list">
      <thead>
         <tr>
            {foreach from=$columns key=col item=name}
               {if $col ne 'TITLE_URL'}
                  <th class="listHeader" id="{$col}"><a href="{$smarty.const.DOC_ROOT}/article_payment.php{if isset($columnURLs) and is_array($columnURLs)}?{$columnURLs.$col}{/if}" title="{l}Sort by{/l}: {$name|escape|trim}" class="sort{if $SORT_FIELD eq $col and $requestOrder eq 1} {$smarty.const.SORT_ORDER}{/if}">{$name|escape|trim}</a></th>
               {/if}
            {/foreach}
      ...
      <tbody>
         {foreach from=$list item=row key=payment_id}
                  ...
                  <td class="status-{$val}">
                     ...
                     <div class="pop-status" id="pS{$payment_id}">
                        ...
                        {foreach from=$stats item=v key=k}
                           {if $k ne $val}
                              <li><a href="{$smarty.const.DOC_ROOT}/article_edit.php?action=S:{$id}:{$k}" class="new-status-{$k}" title="{l}Click to change status to{/l}: {$stats[$k]|escape|trim}">{$stats[$k]|escape|trim}</a></li>
                           {/if}
                        {/foreach}
                        </ul>
                     </div>
                  {elseif $col eq 'TITLE'}
                     <td class="item-title">{$row.$col|escape|trim}<a href="{$smarty.const.DOC_ROOT}/article_details.php?id={$id}" title="{l}View full info of this item{/l}" id="more-info-article-{$id}" class="more-info-article" onclick="return false;"><span>{l}More info{/l}</span></a>
                   {elseif $col eq 'ARTICLE_TYPE'}                
                     <td>{$article_type_str.$val|escape}
                   ...
                   <div class="pop-status" id="{$payment_id}000{$payment_id}">
                        ...
                        {foreach from=$paystatus item=v key=k}
                           {if $k ne $val}
                              ...
                              <li><a href="{$smarty.const.DOC_ROOT}/article_payment.php?action=S:{$payment_id}:{$k}" class="new-status-{if $k eq 1}1{elseif $k eq 2}2{elseif $k eq 3}0{else}0{/if}" title="{l}Click to change status to{/l}: {$paystatus[$k]|escape|trim}">{$paystatus[$k]|escape|trim}</a></li>
                           {/if}
                        {/foreach}
                        </ul>
                     </div>
            ...
            {/foreach}
            <td class="noborder"><a id="edit-link-{$id}-payment-{$payment_id}" href="{$smarty.const.DOC_ROOT}/article_edit.php?action=E:{$id}" title="{l}Edit Article{/l}: {$row.TITLE|escape|trim}" class="action edit"><span>{l}Edit{/l}</span></a></td>
            <td class="noborder last-child"><a id="remove-payment-{$payment_id}" href="{$smarty.const.DOC_ROOT}/article_payment.php?action=D:{$payment_id}" onclick="return payment_rm_confirm('{escapejs}{l}Are you sure you want to remove this payment?{/l}\n{l}Note: payment listings can not be restored after removal!{/l}{/escapejs}');" title="{l}Remove Payment{/l}" class="action delete"><span>{l}Delete{/l}</span></a></td>
         </tr>
      {foreachelse}
            <tr>
