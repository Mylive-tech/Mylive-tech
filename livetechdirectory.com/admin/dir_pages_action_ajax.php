...
switch ($submitAction) {
	...
	case 'remove' :
         if (!empty ($selectedLinks))
         {
            ...
            {
               $ActionStatus = $db->Execute("DELETE FROM `{$tables['page']['name']}` WHERE `ID` = ".$db->qstr($linkID));
            }
         }
         break;
   ...
   case 'title':
   		...
   		$id = $_POST['id'];
   		$value = $_POST['value'];
   		$ActionStatus['status'] = $db->Execute("UPDATE `{$tables['page']['name']}` SET `TITLE` = '{$value}' WHERE `ID` = ".$db->qstr($id));
   		$callback_data = $value;
   	break;
   ...
   default :
      ...
      if (!empty ($selectedLinks) && in_array ($newStatus, $validLinkStatus))
      {
         foreach ($selectedLinks as $key => $linkID)
         {
            $ActionStatus = SetNewPageStatus($linkID, $newStatus);
         }
      }
      break;
}
