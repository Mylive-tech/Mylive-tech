...
require_once 'revlink_multi_action.php';
...
$columns = array ('TITLE' => _L('Title')   , 'URL' => _L('URL')    , 'DESCRIPTION' => _L('Description'), 'CATEGORY' => _L('Category'), 'RECPR_URL' => _L('Recpr. Link URL'), 'DATE_ADDED' => _L('Date Added'));
...
$tpl->assign('col_count', count ($columns) + 5);
...
if (!empty ($linkID))
{
   ...
   {
      $where .= " AND {$tables['link_review']['name']}.LINK_ID IN ('".implode ("', '", $linkIDArray)."')";
...
if (!empty ($category))
{
   ...
   {
      $where .= " AND {$tables['link_review']['name']}.CATEGORY_ID IN ('".implode ("', '", $categsArray)."')";
...
//if (!empty ($searchByID))
//{
//   $where .= " AND {$tables['link_review']['name']}.LINK_ID LIKE ".$db->qstr('%'.$searchByID.'%');
//   $_REQUEST['searchbyid'] = $searchByID;
//}

$orderBy = "{$tables['link_review']['name']}.FEATURED DESC";
if (defined ('SORT_FIELD') && SORT_FIELD != '')
   $orderBy .= ", ". (SORT_FIELD == "CATEGORY" ? "{$tables['category']['name']}.TITLE" : "{$tables['link_review']['name']}.".SORT_FIELD)." ".SORT_ORDER;

$left_join_categ = " LEFT JOIN `{$tables['category']['name']}` ON ({$tables['link_review']['name']}.CATEGORY_ID = {$tables['category']['name']}.ID) ";
...
$search_preferences['Search_Location'] = array ( "{$tables['link_review']['name']}.URL" ,
                                                 "{$tables['link_review']['name']}.TITLE",
                                                 "{$tables['link_review']['name']}.DESCRIPTION",
                                                  "{$tables['link_review']['name']}.ID"
                                                 );
...
if ($_SESSION['phpld']['adminpanel']['is_admin'])
{
   $list_total = $db->GetOne("SELECT COUNT(*) FROM `{$tables['link_review']['name']}` WHERE 1 ".(!empty ($LinksResults['Search_Query']) ? " AND ".$LinksResults['Search_Query'] : '')." {$where}");
   $tpl->assign('list_total', $list_total);

   $sql = "SELECT {$tables['link_review']['name']}.*, ".$db->IfNull("{$tables['category']['name']}.TITLE", "'Top'")." AS `CATEGORY` ".(!empty ($LinksResults['Select_Relevancy']) ? ", ".$LinksResults['Select_Relevancy'] : '')." FROM `{$tables['link_review']['name']}` LEFT JOIN `{$tables['category']['name']}` ON ({$tables['link_review']['name']}.CATEGORY_ID = {$tables['category']['name']}.ID) WHERE 1 ".(!empty ($LinksResults['Search_Query']) ? " AND ".$LinksResults['Search_Query'] : '').(!empty ($LinksResults['Relevancy_Tuning']) ? $LinksResults['Relevancy_Tuning'] : '')." {$where} ORDER BY ".(!empty ($LinksResults['Relevancy_Order']) ? $LinksResults['Relevancy_Order'].", " : '')." {$orderBy}";
...
{
   $list_total = $db->GetOne("SELECT COUNT(*) FROM `{$tables['link_review']['name']}` {$left_join_categ} WHERE 1 ".(!empty ($LinksResults['Search_Query']) ? " AND ".$LinksResults['Search_Query'] : '')." {$where} AND (".preg_replace ('`(?:^|\s)ID(?:$|\s|=)`'," {$tables['link_review']['name']}.CATEGORY_ID ",$_SESSION['phpld']['adminpanel']['permission']).")");

   $sql = "SELECT {$tables['link_review']['name']}.*, ".$db->IfNull("{$tables['category']['name']}.TITLE", "'Top'")." AS `CATEGORY` ".(!empty ($LinksResults['Select_Relevancy']) ? ", ".$LinksResults['Select_Relevancy'] : '')." FROM `{$tables['link_review']['name']}` {$left_join_categ} WHERE 1 ".(!empty ($LinksResults['Search_Query']) ? " AND ".$LinksResults['Search_Query'] : '').(!empty ($LinksResults['Relevancy_Tuning']) ? $LinksResults['Relevancy_Tuning'] : '')." {$where} AND (".preg_replace ('`(?:^|\s)ID(?:$|\s|=)`'," {$tables['link_review']['name']}.CATEGORY_ID ",$_SESSION['phpld']['adminpanel']['permission']).") ORDER BY ".(!empty ($LinksResults['Relevancy_Order']) ? $LinksResults['Relevancy_Order'].", " : '')." {$orderBy}";
...
$content = $tpl->fetch(ADMIN_TEMPLATE.'/dir_reviewed_links.tpl');
