...
//Determine columns
$columns = array ('ID', 'ORDER_ID', 'NAME', 'PRICE', 'STATUS', 'LIST_TEMPLATE', 'DETAILS_TEMPLATE', 'ACTION', 'CORE_LINK');

$stats = array (0 => _L('Inactive'), 2 => _L('Active'));
$valid = array (0 => _L('Broken')  , 1 => _L('Unknown'), 2 => _L('Ok'),);
$yes_no = array (1 => _L('Yes'), 0 => _L('No'));
$link_types = $db->GetAssoc("SELECT `ID`, `NAME` FROM `{$tables['link_type']['name']}`");
...
if ( $_GET['sSearch'] != "" ) {
	...
	$where .= "AND ( {$tables['link_type']['name']}.NAME LIKE '%{$search_text}%' ";
}
...
$list_total = $db->GetOne("SELECT COUNT(*) FROM `{$tables['link_type']['name']}` WHERE 1 {$where}");

$sql = "SELECT SQL_CALC_FOUND_ROWS {$tables['link_type']['name']}.* FROM `{$tables['link_type']['name']}`  WHERE 1 {$where}  {$sOrder}";
