...
if ( $_GET['sSearch'] != "" ) {
	...
	$sWhere = "WHERE {$tables['lang']['name']}.VALUE LIKE '%{$search_text}%'";
}
...
if ($_SESSION['phpld']['adminpanel']['is_admin']) {
	...
	$list_total = $db->GetOne("SELECT COUNT(*) FROM `{$tables['lang']['name']}` {$sWhere}");

   $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM `{$tables['lang']['name']}` {$sWhere} {$sOrder}";
...
$json = lang_json($list, $columns, $list_total, $filtered_total);

echo $json;

function lang_json($links, $columns, $total_count, $display_count) {
