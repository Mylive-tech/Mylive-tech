...
$selectedCategs = (!empty ($_POST['multiselect_checkbox']) && is_array ($_POST['multiselect_checkbox']) ? $_POST['multiselect_checkbox'] : array());
...
if (!empty ($submitAction))
{
   switch ($submitAction)
   {
      case 'changeparent':
         ...
         if (!empty ($selectedCategs) && $parentID >= 0)
         {
      ...
      default :
         ...
         if (!empty ($selectedCategs) && in_array ($newStatus, $validCategStatus))
         {
            foreach ($selectedCategs as $key => $categID)
            {
               $ActionStatus = SetNewCategoryStatus($categID, $newStatus);
               $thisError = ($ActionStatus['status'] == 1 ? 0 : 1);
