...
switch ($submitAction) {
   ...
   default :
      $newStatus = -1;
      if ($submitAction == 'active') $newStatus = 1;
      elseif ($submitAction == 'pending') $newStatus = 0;
      ...
      if (!empty ($selectedLinks) && in_array ($newStatus, $validLinkStatus))
      {
         ...
         {
            $ActionStatus = $db->Execute("UPDATE `{$tables['inline_widget']['name']}` SET `STATUS` = '{$newStatus}' WHERE `ID` = '{$linkID}'");
         }
      }
      break;
}
