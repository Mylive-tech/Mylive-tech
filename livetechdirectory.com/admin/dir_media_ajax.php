...
//Determine columns
$columns = array ('ID', 'FILE_NAME', 'TYPE' , 'OWNER_NAME' , 'DATE_ADDED', 'ACTION');
...
if ( $_GET['sSearch'] != "" ) {
	...
	$where .= "AND ( {$tables['media_manager_items']['name']}.FILE_NAME LIKE '%{$search_text}%' OR ".
		                	"{$tables['media_manager_items']['name']}.TYPE LIKE '%{$search_text}%') ";
}
...
$list_total = $db->GetOne("SELECT COUNT(*) FROM `{$tables['media_manager_items']['name']}` WHERE 1 {$where}");

$sql = "SELECT SQL_CALC_FOUND_ROWS {$tables['media_manager_items']['name']}.*, ".$db->IfNull("{$tables['user']['name']}.NAME", "'Admin'")." AS `OWNER_NAME`
		FROM `{$tables['media_manager_items']['name']}` LEFT JOIN `{$tables['user']['name']}` ON ({$tables['media_manager_items']['name']}.USER_ID = {$tables['user']['name']}.ID)
		WHERE 1 {$where}  {$sOrder}";
		...
		foreach ($columns as $col_id => $col_name) {
			...
			switch ($col_name) {
				...
				case 'PAGERANK':
					...
					break; 
				case 'DATE_ADDED':
					$val = strftime("%m-%d-%y",strtotime($val));
					break;
