 ...
 * Type:     function
 * Name:     formtool_copy
 * Purpose:  copy selected element(s) to another field
 * -------------------------------------------------------------
 */
function smarty_function_formtool_copy($params, &$smarty)
{
    // required parameters
    foreach (array('from', 'to', 'save') as $item) {
        if (!array_key_exists($item, $params) || empty($params[$item])) {
            $smarty->trigger_error("formtool_copy: missing '$item' parameter");
            return;
        } else {
            $local = "_$item";
    ...
    return "<input$_class$_style type=\"button\" value=\"$_button_text\" onclick=\"javascript:formtool_copy($_from,$_to,$_save,$_counter,$_all);\" />\n";
}
