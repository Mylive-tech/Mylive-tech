	...
	*/
	...
	GLOBAL 
		...
		if (!isset($ADODB_CACHE_DIR)) {
			// Dmitriy change releated with php path when using cron
			$server_cache_dir = dirname(__FILE__);
			$server_cache_dir = substr($server_cache_dir, 0, strrpos($server_cache_dir, '/libs/adodb'));
			$server_cache_dir = $server_cache_dir.'/temp/adodb';
			
			$ADODB_CACHE_DIR = $server_cache_dir;
...
*/
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	function SelectLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$secs2cache=0)
	{
		...
		if ($secs2cache != 0) $rs = $this->Execute($sql,$inputarr);
	...
	*/
	...
	function CacheGetAssoc( $sql=false, $inputarr=false,$force_array = false, $first2cols = false)
	{
	...
	*/
	...
	function CacheGetOne($sql,$inputarr=false)
	{
	global $ADODB_COUNTRECS,$ADODB_GETONE_EOF;
		$crecs = $ADODB_COUNTRECS;
		$ADODB_COUNTRECS = false;
	...
	function CacheGetCol($sql, $inputarr = false, $trim = false)
	{
	...
	*/
	...
	function CacheGetAll($nRows = -1)
	{
		$arr = $this->GetArray($nRows);
		return $arr;
	}
	
	function CacheGetArray($sql,$inputarr=false)
	{
	global $ADODB_COUNTRECS;
	...
	*/
	...
	function CacheGetRow($sql,$inputarr=false)
	{
	global $ADODB_COUNTRECS;
		$crecs = $ADODB_COUNTRECS;
		$ADODB_COUNTRECS = false;
	...
	* @return		the recordset ($rs->databaseType == 'array')
 	*/
	function CacheSelectLimit($sql,$nrows=-1,$offset=-1,$inputarr=false)
	{	
...
*/
	...
	* @return		the recordset ($rs->databaseType == 'array')
	*/
	function CachePageExecute($sql, $nrows, $page, $inputarr=false, $secs2cache=0) 
	{
		global $ADODB_INCLUDED_LIB;
		if (empty($ADODB_INCLUDED_LIB)) include(ADODB_DIR.'/adodb-lib.inc.php');
		if ($this->pageExecuteCountRows) $rs = _adodb_pageexecute_all_rows($this, $sql, $nrows, $page, $inputarr, $secs2cache);
