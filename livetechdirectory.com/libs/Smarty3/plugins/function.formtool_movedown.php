 ...
 * -------------------------------------------------------------
 * Type:     function
 * Name:     formtool_movedown
 * Purpose:  move selected element(s) down in a list
 * -------------------------------------------------------------
 */
function smarty_function_formtool_movedown($params, &$smarty)
{
    // required parameters
    foreach (array('name', 'save') as $item) {
        if (!array_key_exists($item, $params) || empty($params[$item])) {
            $smarty->trigger_error("formtool_movedown: missing '$item' parameter");
            return;
        } else {
            $local = "_$item";
    ...
    $_button_text = isset($params['button_text']) ? $params['button_text'] : 'Move Down';

    return "<input$_class$_style type=\"button\" value=\"$_button_text\" onclick=\"javascript:formtool_movedown($_name,$_save);\" />\n";
}
